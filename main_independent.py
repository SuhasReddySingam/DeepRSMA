...
class KervNet(nn.Module):
    def __init__(self, in_channels, out_channels, num_centroids=10, dropout_prob=0.2):
        super().__init__()
        self.lin1 = nn.Linear(in_channels, num_centroids)
        self.centroids = nn.Parameter(torch.randn(num_centroids, in_channels))
        self.gamma = nn.Parameter(torch.tensor(0.1))
        self.lin2 = nn.Linear(num_centroids, out_channels)
        self.relu = nn.ReLU()
        self.dropout = nn.Dropout(dropout_prob)
        self.layer_norm = LayerNorm(out_channels)

    def forward(self, x):
        z = self.lin1(x)  # (batch_size, num_centroids)
        dist = torch.cdist(z, self.centroids)  # (batch_size, num_centroids)
        h = torch.exp(-self.gamma * dist ** 2)  # Gaussian kernel
        h = self.relu(h)
        h = self.dropout(h)
        out = self.lin2(h)  # (batch_size, out_channels)
...
class DeepRSMA(nn.Module):
    def __init__(self):
        ...
        self.line1 = KervNet(hidden_dim*2, 1024, num_centroids=10, dropout_prob=0.2)
        self.line2 = KervNet(1024, 512, num_centroids=10, dropout_prob=0.2)
        self.line3 = KervNet(512, 1, num_centroids=10, dropout_prob=0.2)
        self.dropout = nn.Dropout(0.2)
        
        self.rna1 = KervNet(hidden_dim, hidden_dim*4, num_centroids=10, dropout_prob=0.2)
        self.mole1 = KervNet(hidden_dim, hidden_dim*4, num_centroids=10, dropout_prob=0.2)
        
        self.rna2 = KervNet(hidden_dim*4, hidden_dim, num_centroids=10, dropout_prob=0.2)
        self.mole2 = KervNet(hidden_dim*4, hidden_dim, num_centroids=10, dropout_prob=0.2)
